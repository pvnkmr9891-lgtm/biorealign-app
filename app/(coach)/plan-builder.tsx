...
// Radius/shadow tokens only — BRAND keeps its own (correctly-loaded) font
// names and color tuning, see the note on the BRAND const below.
const R2XL = THEME.radius['2xl'];
const RXL  = THEME.radius.xl;
function glowShadow(color: string, opacity = 0.3) {
  return { shadowColor: color, shadowOffset: { width: 0, height: 0 }, shadowOpacity: opacity, shadowRadius: 14, elevation: 5 };
}
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'flex-end' }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={{ backgroundColor: BRAND.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, maxHeight: '90%' }}>
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <TextInput style={{ backgroundColor: '#1A1A1E', borderRadius: RXL, paddingHorizontal: 14, paddingVertical: 12, color: BRAND.text, fontFamily: 'DMSans-Regular', fontSize: 15, marginBottom: 16 }} value={title} onChangeText={setTitle} placeholder="e.g. Goblet Squat" placeholderTextColor={BRAND.textMuted} maxLength={MAX_LENGTHS.shortTitle} />
          ...
          {itemType === 'exercise' && (
            <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
              <View style={{ flex: 1 }}>
                ...
                <TextInput style={{ backgroundColor: '#1A1A1E', borderRadius: RXL, paddingHorizontal: 14, paddingVertical: 12, color: BRAND.text, fontFamily: 'DMSans-Regular', fontSize: 15 }} value={sets} onChangeText={(t) => setSets(sanitizeInteger(t))} placeholder="3" placeholderTextColor={BRAND.textMuted} keyboardType="numeric" maxLength={3} />
              </View>
              <View style={{ flex: 1 }}>
                ...
                <TextInput style={{ backgroundColor: '#1A1A1E', borderRadius: RXL, paddingHorizontal: 14, paddingVertical: 12, color: BRAND.text, fontFamily: 'DMSans-Regular', fontSize: 15 }} value={reps} onChangeText={setReps} placeholder="12" placeholderTextColor={BRAND.textMuted} maxLength={20} />
              </View>
              <View style={{ flex: 1 }}>
                ...
                <TextInput style={{ backgroundColor: '#1A1A1E', borderRadius: RXL, paddingHorizontal: 14, paddingVertical: 12, color: BRAND.text, fontFamily: 'DMSans-Regular', fontSize: 15 }} value={duration} onChangeText={(t) => setDuration(sanitizeInteger(t))} placeholder="30" placeholderTextColor={BRAND.textMuted} keyboardType="numeric" maxLength={3} />
              </View>
            </View>
          )}
          ...
          <TouchableOpacity onPress={handleAdd} disabled={addItem.isPending}
            style={{ backgroundColor: BRAND.teal, borderRadius: R2XL, paddingVertical: 16, alignItems: 'center', ...glowShadow(BRAND.teal, 0.4) }}>
...
function AISuggestionPanel({
  ...
  if (!suggestion && !isLoading && !error) {
    return (
      <FadeInUp delay={40} style={{ marginHorizontal: 16, marginBottom: 20, backgroundColor: `${BRAND.teal}10`, borderRadius: R2XL, padding: 20, ...glowShadow(BRAND.teal, 0.15) }}>
        ...
        <TouchableOpacity onPress={() => generate(clientId, clientName)}
          style={{ backgroundColor: BRAND.teal, borderRadius: RXL, paddingVertical: 14, alignItems: 'center', ...glowShadow(BRAND.teal, 0.4) }}>
  ...
  if (isLoading) {
    return (
      <View style={{ marginHorizontal: 16, marginBottom: 20, backgroundColor: `${BRAND.teal}10`, borderRadius: R2XL, padding: 24, alignItems: 'center', ...glowShadow(BRAND.teal, 0.15) }}>
  ...
  if (error) {
    return (
      <View style={{ marginHorizontal: 16, marginBottom: 20, backgroundColor: '#F8717115', borderRadius: R2XL, padding: 20, ...glowShadow('#F87171', 0.2) }}>
        <Text style={{ fontSize: 14, fontFamily: 'DMSans-Medium', color: '#F87171', marginBottom: 12 }}>⚠️ {error}</Text>
        <View style={{ flexDirection: 'row', gap: 10 }}>
          <TouchableOpacity onPress={() => generate(clientId, clientName)}
            style={{ flex: 1, backgroundColor: BRAND.teal, borderRadius: RXL, paddingVertical: 12, alignItems: 'center' }}>
            <Text style={{ fontSize: 14, fontFamily: 'DMSans-Bold', color: '#0A0A0B' }}>Try again</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => { reset(); onDismiss(); }}
            style={{ flex: 1, backgroundColor: BRAND.surface, borderRadius: RXL, paddingVertical: 12, alignItems: 'center' }}>
  ...
  return (
    <View style={{ marginHorizontal: 16, marginBottom: 20 }}>
      {/* Summary card */}
      <View style={{ backgroundColor: `${BRAND.teal}12`, borderRadius: R2XL, padding: 16, marginBottom: 12, ...glowShadow(BRAND.teal, 0.15) }}>
      ...
      {suggestion.tracks.map((track: AISuggestedTrack, trackIdx: number) => {
        ...
        return (
          <View key={trackIdx} style={{ backgroundColor: BRAND.surface, borderRadius: R2XL, marginBottom: 10, overflow: 'hidden', opacity: rejected ? 0.4 : 1, ...glowShadow(accepted ? BRAND.teal : rejected ? '#F87171' : '#000', accepted ? 0.35 : rejected ? 0.2 : 0.15) }}>
            ...
            <View style={{ padding: 10, gap: 6 }}>
              {track.items.map((item: any, i: number) => (
                <View key={i} style={{ backgroundColor: '#1A1A1E', borderRadius: RXL, padding: 10 }}>
            ...
            {!rejected && (
              <View style={{ flexDirection: 'row', gap: 8, padding: 10, paddingTop: 0 }}>
                <TouchableOpacity
                  ...
                  disabled={accepted}
                  style={{ flex: 1, backgroundColor: accepted ? `${BRAND.teal}30` : BRAND.teal, borderRadius: RXL, paddingVertical: 10, alignItems: 'center' }}>
                ...
                <TouchableOpacity
                  ...
                  style={{ backgroundColor: BRAND.surface, borderRadius: RXL, paddingVertical: 10, paddingHorizontal: 16, borderWidth: 0.5, borderColor: BRAND.border }}>
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: BRAND.bg }}>
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 60 }} keyboardShouldPersistTaps="handled">
            ...
            <TextInput style={{ backgroundColor: BRAND.surface, borderRadius: RXL, paddingHorizontal: 16, paddingVertical: 14, color: BRAND.text, fontFamily: 'DMSans-Regular', fontSize: 16, marginBottom: 20 }}
            ...
            <TextInput style={{ backgroundColor: BRAND.surface, borderRadius: RXL, paddingHorizontal: 16, paddingVertical: 14, color: BRAND.text, fontFamily: 'DMSans-Regular', fontSize: 15, minHeight: 100, marginBottom: 32 }}
            ...
            <TouchableOpacity onPress={handleCreatePlan} disabled={createPlan.isPending}
              style={{ backgroundColor: BRAND.teal, borderRadius: R2XL, paddingVertical: 18, alignItems: 'center', ...glowShadow(BRAND.teal, 0.4) }}>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: BRAND.bg }}>
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        {/* Header */}
        <FadeInUp delay={0} style={{ padding: 24, paddingBottom: 16 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 6 }}>
              {[1, 2, 3].map(p => (
                <TouchableOpacity key={p} onPress={() => setPhase(p)}
                  style={[
                    { width: 32, height: 32, borderRadius: 16, backgroundColor: phase === p ? BRAND.teal : BRAND.surface, alignItems: 'center', justifyContent: 'center' },
                    phase === p ? glowShadow(BRAND.teal, 0.5) : { borderWidth: 1, borderColor: BRAND.border },
                  ]}>
        ...
        {planLoading ? (
          ...
          <View style={{ paddingHorizontal: 16 }}>
            {(planDetail?.plan_tracks ?? []).map((track: any, ti: number) => {
              const meta       = TRACK_OPTIONS.find(t => t.type === track.track_type)!;
              const trackItems = (track.plan_items ?? []).filter((i: any) => i.phase === phase);
              const trackColor = meta?.color ?? BRAND.teal;

              return (
                <FadeInUp key={track.id} delay={Math.min(ti, 6) * 50} style={{ backgroundColor: BRAND.surface, borderRadius: R2XL, marginBottom: 16, overflow: 'hidden', ...glowShadow('#000', 0.3) }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', padding: 16, borderBottomWidth: 0.5, borderBottomColor: BRAND.border }}>
                    <View style={{ width: 4, height: 36, borderRadius: 2, backgroundColor: trackColor, marginRight: 12 }} />
                    <Text style={{ fontSize: 16, marginRight: 8 }}>{meta?.emoji}</Text>
                    ...
                    <TouchableOpacity onPress={() => { setActiveTrackId(track.id); setShowAddItem(true); }}
                      style={{ backgroundColor: `${BRAND.teal}20`, borderRadius: THEME.radius.full, paddingHorizontal: 12, paddingVertical: 6, marginRight: 8 }}>
                  ...
                  {trackItems.length === 0 ? (
                    ...
                    <View style={{ padding: 12, gap: 8 }}>
                      {trackItems.map((item: any) => (
                        <View key={item.id} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1E', borderRadius: RXL, padding: 12 }}>
                          <View style={{ flex: 1 }}>
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                  {availableTracks.map(t => (
                    <TouchableOpacity key={t.type} onPress={() => handleAddTrack(t.type)} disabled={addTrack.isPending}
                      style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: BRAND.surface, borderRadius: THEME.radius.full, paddingHorizontal: 14, paddingVertical: 10, gap: 6, ...glowShadow('#000', 0.25) }}>
      ...
      {planDetail?.status !== 'active' && (
        <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: BRAND.bg, padding: 24, paddingBottom: 36, ...glowShadow('#000', 0.4), shadowOffset: { width: 0, height: -6 } }}>
          <TouchableOpacity onPress={handlePublish} disabled={publishPlan.isPending || !planDetail?.plan_tracks?.length}
            style={{ backgroundColor: BRAND.teal, borderRadius: R2XL, paddingVertical: 18, alignItems: 'center', ...glowShadow(BRAND.teal, 0.4), opacity: planDetail?.plan_tracks?.length ? 1 : 0.5 }}>
